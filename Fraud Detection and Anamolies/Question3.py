...
import scipy 
from numpy import linalg as LG1
from scipy import linalg as LG2
...
#seperate the data into two using fraud variable

fraud_data = trainData[trainData['FRAUD']==1]
not_fraud =  trainData[trainData['FRAUD']==0]

#calculate number of observations in each sample stratum

numoffrauds = len(fraud_data)
numofnotfrauds = len(not_fraud)
...
print("Training Frauds",train_fraud)
print("Test Frauds",test_fraud)
print("Train Not Frauds",train_not_fraud)
print("Test not frauds",test_not_fraud)
num_train_obs = len(train_fraud) + len(train_not_fraud)
num_test_obs = len(test_fraud) + len(test_not_fraud)

print("Total training observations",num_train_obs)
print("Total testing observation",num_test_obs)

from sklearn.preprocessing import StandardScaler
from sklearn.metrics import accuracy_score

#take the imported dataset 
#traindata for fraud 
...
# Separate the data into two strata based on FRAUD variable
fraud_data = data[data['FRAUD'] == 1]
not_fraud_data = data[data['FRAUD'] == 0]

# Function to calculate empirical fraud rate
def empirical_fraud_rate(data):
    return round((data['FRAUD'] == 1).mean(), 4)

# Calculate the empirical fraud rate
empirical_fraud_rate_all = empirical_fraud_rate(data)

# Initialize a dictionary to store misclassification rates
misclassification_rates = {}
...
# Initialize variables to track the best number of neighbors and the lowest misclassification rate
best_k = None
...
# Extract features and target variables for the entire dataset
X = data[['DOCTOR_VISITS', 'MEMBER_DURATION', 'NUM_CLAIMS', 'NUM_MEMBERS', 'OPTOM_PRESC', 'TOTAL_SPEND']]
y = data['FRAUD']
...
# Initialize the KNN classifier with the selected K
